...
class TransformerParser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument("xmi_path", help="Path where the 'source' XMIs reside.")
        self.add_argument("typesystem", help="Path to the 'source' Typesystem.")
        self.add_argument(
            "mapping_file",
    ...
    def parse_args(self, **kwargs):
        ...
        _exp_dict = {
            "json": ExportEnum.JSON,
            "jason": ExportEnum.JSON,
            "jsn": ExportEnum.JSON,
            "xmi": ExportEnum.XMI,
            "xml": ExportEnum.XMI,
        }
        if ns.export_format not in _exp_dict:
            logging.warning(
                f"Unknown export format: {ns.export_format}. Using default format 'json'."
            )
...
def check_for_more_specific(
    duplicate_dict: dict, layer_instance: FeatureStructure, feat
):
    _kind = (
        layer_instance.get("kind")
        if layer_instance.get("kind") is not None
        else layer_instance.type.name
    )
    ...
    if _id not in duplicate_dict:
        ...
        _old_none = [
            duplicate_dict[_id]["layer"].get(i)
            for i in duplicate_dict[_id]["layer"].__dir__()
            if (not i.startswith("__"))
        ].count(None)

        _now_none = [
            layer_instance.get(i)
            for i in layer_instance.__dir__()
            if (not i.startswith("__"))
        ].count(None)
...
def mark_new(
    ...
    for source_layer, mapping_dict in mapping.annotation_mapping.items():
                ...
                if mapping_dict.mapping_type == MappingTypeEnum.SINGLELAYER:
                    for k, v in mapping_dict.items():
                        if v(layer_instance):
                            ...
                            _dupl = mapping_dict.additional_feats.pop(
                                mapping_dict.target_feature, None
                            )
                            if _dupl is not None:
                                logging.warning(
                                    f"Removed {_dupl} from 'add_feature' for entry '{mapping_dict.entry_name}_{k}'."
                                )
                            feat.update(
                                {
                                    tf: sf[0](layer_instance, sf[1])
                                    for tf, sf in mapping_dict.additional_feats.items()
                                }
                            )
                            break
                else:
                    feat = {
                        tf: sf[0](layer_instance, sf[1])
                        for tf, sf in mapping_dict.items()
                    }
        ...
        for fs_dict in duplicate_check.values():
            ...
            ts_instance = ts_init(
                begin=_layers.get("begin"), end=_layers.get("end"), **_feats
            )
...
def init_source_cas(
    ...
    try:
        if cas_path.is_file():
            ...
            logging.info(
                f" Processing {suffix}-data from:"
                f" '{cas_path.resolve()}' ({_count} file{'' if _count == 1 else 's'})."
            )
            for cas_file in tqdm(
                cas_path.glob(f"*.{suffix}"), ascii=True, total=_count
            ):
...
if __name__ == "__main__":
    ...
    with logging_redirect_tqdm():
        for _cas, _ts, fi_name in init_source_cas(
            args.xmi_path, args.typesystem, args.file_ending
        ):
            target_cas = Cas(target_ts)
            export_cas, missing_types = mark_new(
                target_cas, _cas, target_ts, mapping_config, missing_types
            )
            if args.export_format == ExportEnum.JSON:
                export_cas.to_json(
                    pathlib.Path(args.output_path, f"{fi_name}.json"), pretty_print=True
                )
            elif args.export_format == ExportEnum.XMI:
                export_cas.to_xmi(
                    pathlib.Path(args.output_path, f"{fi_name}.xmi"), pretty_print=True
                )

...
class TestRf2Hdf5Ingestion(unittest.TestCase):
    ...
    def test_write_snapshot_hdf5_with_historical_associations(self):
        ...
        self.assertEqual(summary.fsn_count, 3)
        self.assertEqual(summary.association_count, 1)
        self.assertEqual(summary.whitelist_count, 2)
        self.assertEqual(summary.blacklist_count, 2)
        self.assertEqual(concept_codes, ["100", "200", "300"])
        self.assertEqual(fsns, ["Old thing (finding)", "New thing (finding)", "Inactive thing (finding)"])
        self.assertEqual(semantic_tags, ["finding"])
        self.assertEqual(semantic_tag_ids, [0, 0, 0])
        self.assertEqual(active, [True, True, False])
        self.assertEqual(source_indices, [2])
        self.assertEqual(target_indices, [1])
        self.assertEqual(association_types, ["REPLACED_BY"])
        self.assertEqual(association_type_ids, [0])
        self.assertEqual(whitelist_indices, [0, 1])
        self.assertEqual(blacklist_indices, [0, 1])
        self.assertEqual(legacy_whitelist_codes, ["100", "200"])
        self.assertEqual(legacy_blacklist_codes, ["100", "200"])

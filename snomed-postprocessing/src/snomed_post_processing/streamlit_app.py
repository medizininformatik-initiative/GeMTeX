...
from snomed_post_processing.inception_io import get_project_zip
...
check_tab, sanitization_tab = st.tabs(["1. Check whitelist/blacklist", "2. Sanitization suggestions"])
...
with check_tab:
    ...
    if st.button("Run policy check", type="primary", disabled=not (st.session_state.get("zip_file") and hdf5_file)):
        try:
...
with sanitization_tab:
    st.write("Generate sanitization suggestions from CriticalFindings JSON produced by the policy check step.")
    ...
    if st.button(
        ...
        type="primary",
        disabled=not hdf5_file or not (use_session_findings or uploaded_findings_file),
    ):
        try:
            if hdf5_temp_path is None:
                hdf5_temp_path = save_uploaded_file(hdf5_file, ".hdf5")
            ...
            sanitization_progress = st.progress(0.0, text="Preparing sanitization suggestions...")
            ...
            sanitization_progress.progress(0.25, text=f"Resolving historical-association suggestions for {len(findings)} finding(s)...")
            suggestions = resolver.suggest_all(findings)
            if sanitize_semantic_bm25_fallback:
                sanitization_progress.progress(0.65, text="Running semantic BM25 fallback suggestions...")
            ...
            sanitization_progress.progress(0.9, text="Writing sanitization suggestion report...")
            output_sanitization_md = pathlib.Path(tempfile.mkdtemp(prefix="snomed_gui_sanitization_")) / (
            ...
            sanitization_progress.progress(1.0, text="Sanitization suggestions finished.")
            time.sleep(0.2)

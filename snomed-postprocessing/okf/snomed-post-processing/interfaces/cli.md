...
The command imports annotation, curation, and flat CAS views into one SQLite file. It normalizes document names, infers site and batch metadata from names such as `berlin_XMI_1-3.zip`, enriches known SCTIDs with FSN/semantic tag/active status from `/concepts`, keeps unknown SCTIDs with null metadata, stores a SHA-256 hash of each complete CAS document text for reproducibility, and reports missing batches. Full CAS document text is only stored with `--store-document-text`; covered text is always stored per annotation occurrence. Filename inference accepts `<SITE>_XMI_<PART>-<TOTAL>.zip` or `<SITE>_JSON_<PART>-<TOTAL>.zip`, corresponding `_flat_` variants, optional descriptive suffixes after the batch marker, and unsplit `<SITE>_XMI.zip` / `<SITE>_JSON.zip` forms. Use `--site`, `--batch-index`, and `--batch-total` for explicit overrides. The creation summary reports unknown identifiers as `Unknown SCTIDs: N (M missing/empty/null-like ids)` and failed CAS members as `Failed CAS members: N (of which serialized CAS: M)`; `.ser` members are unsupported and counted in the serialized-CAS subtotal.

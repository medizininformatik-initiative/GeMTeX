...
def examine_cas(cas, cas_name):
    ...
    return (
        {kind: list(set(dict(stats_det[kind]).keys())) for kind in stats_det},
...
def run_quality_control_only(config):
    ...
    dir_out_private, dir_out_public, surrogate_modes, timestamp_key = handle_config(
        config
    )
    ...
    proof_projects(
        projects=projects, dir_out_private=dir_out_private, timestamp_key=timestamp_key
    )


def write_quality_control_report(
    quality_control, dir_project_quality_control, project_name, timestamp_key
):
    """
    ...
    md_report = MdUtils(
        file_name=dir_project_quality_control
        + os.sep
        + "Report_Quality_Control_"
        + project_name
        + "_"
        + timestamp_key
        + ".md",
    ...
    with open(
        file=dir_project_quality_control + os.sep + "quality_control.json",
        mode="w",
        encoding="utf8",
    ) as outfile:
        json.dump(
            quality_control, outfile, indent=2, sort_keys=True, ensure_ascii=False
        )

    pd_corpus = (
        pd.DataFrame(quality_control["corpus_files"], index=["part_of_corpus"])
        .rename_axis("document", axis=1)
        .transpose()
    )
    path_file_corpus_documents = (
        dir_project_quality_control + os.sep + project_name + "_corpus_documents.csv"
    )
    pd_corpus.to_csv(path_file_corpus_documents)

    path_file_corpus_details = (
        dir_project_quality_control + os.sep + project_name + "_corpus_details.csv"
    )
    pd.DataFrame(quality_control["stats_detailed"]).transpose().to_csv(
        path_file_corpus_details
    )
    corpus_details = (
        pd.DataFrame(quality_control["stats_detailed"])
        .transpose()
        .rename_axis("document", axis=1)
    )

    path_file_statistics = (
        dir_project_quality_control + os.sep + project_name + "_statistics.csv"
    )
    pd.DataFrame(quality_control["stats_detailed_cnt"]).transpose().to_csv(
        path_file_statistics
    )

    md_report.write(
        "\n\n"
        + pd.DataFrame(quality_control["stats_detailed_cnt"])
        .transpose()
    ...
    for item in ["OTHER", "PROFESSION", "LOCATION_OTHER", "AGE"]:
        if item in corpus_details.keys():
            md_report.write(
                "\n\n"
                + pd.DataFrame(corpus_details)
                .dropna(subset=[item])[item]
                .transpose()
                .rename_axis("document")
                .to_markdown()
                + "\n\n"
    ...
    corpus_files = pd.DataFrame(
        quality_control["corpus_files"], index=["part_of_corpus"]
    ).transpose()
    md_report.write("### Processed Documents\n\n")
    md_report.write(
        pd.DataFrame(corpus_files[corpus_files["part_of_corpus"] == 1].index)
        .rename_axis("document", axis=0)
        .to_markdown()
        + "\n\n"
    )
    ...
    df_part_of_corpus = pd.DataFrame(
    ...
    df_wrong_annotations_none = (
        pd.DataFrame(quality_control["wrong_annotations_none"])
        .transpose()
        .rename_axis("document & token.id")
    )
    if not df_wrong_annotations_none.empty:
        md_report.write(
            "## Wrong annotations (NONE)\n\n"
    ...
    df_wrong_annotations_other = (
        pd.DataFrame(quality_control["wrong_annotations_other"])
        .transpose()
        .rename_axis("document & token.id")
    )
    if not df_wrong_annotations_other.empty:
        md_report.write(
            "## Wrong annotations (OTHER)\n\n"
    ...
    df_wrong_annotations_date_birth = pd.DataFrame(
        quality_control["wrong_annotations_date_birth"]
    )
    ...
    df_wrong_annotations_date_death = pd.DataFrame(
        quality_control["wrong_annotations_date_death"]
    )
    ...
    md_report.write(
        "## Counts DATE_BIRTH\n\n"
        + pd.DataFrame(quality_control["birthday_cnt"], index=["DATE_BIRTH (#)"])
        .rename_axis("document", axis=0)
        .transpose()
        .to_markdown()
        + "\n\n"
    ...
    logging.info(
        msg='Report quality control of project "'
        + project_name
        + '" in '
        + md_report.file_name
    )
...
def run_quality_control_of_project(project):
    ...
    for i, document in enumerate(project["annotations"]):
        ...
        if relevant_types:
            ...
            stats_det, stats_det_count, is_part_of_corpus = examine_cas(
                cas=cas, cas_name=cas_name
            )
                ...
                if "DATE_BIRTH" in stats_detailed[document].keys():
                    for date_ann in stats_detailed[document]["DATE_BIRTH"]:
                        if get_quarter(date_ann) == "none":
                            wrong_annotations_date_death[document] = {
                                "wrong data annotation": date_ann
                            }
                else:
            ...
            for sentence in cas.select(cas_name):
                for token in cas.select_covered(cas_name, sentence):
                    if token.kind is None or "OTHER" == token.kind:
                        ...
                        logging.warning(
                            msg="---- Wrong Annotation : [kind]"
                            + str(token.kind)
                            + " ----"
                        )
                        logging.warning(msg="token.xmiID: " + str(token.xmiID))
                        logging.warning(
                            msg="token.text: " + str(token.get_covered_text())
                        )
                        logging.warning(
                            msg="token.begin: "
                            + str(token.begin)

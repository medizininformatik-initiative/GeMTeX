...
def set_surrogates_in_inception_projects(config):
    ...
    logging.info(
        msg="surrogate modes: " + str(config["surrogate_process"]["surrogate_modes"])
    )
    ...
    for mode in surrogate_modes:
        if mode in ["x", "entity"]:
            ...
            logging.warning(
                "No valid modus, only x, entity, gemtex and fictive allowed."
            )
            exit()
        ...
        for project in projects:
            ...
            for i, ann_doc in enumerate(
                corpus_documents[corpus_documents["part_of_corpus"] == 1].index
            ):
            ...
            if mode in ["gemtex", "fictive"]:
                ...
                for filename in corpus_documents[
                    corpus_documents["part_of_corpus"] == 1
                ].index:
                    for annotations in doc_random_keys[filename]["annotations"]:
                        for key in doc_random_keys[filename]["annotations"][
                            annotations
                        ]:
                            flat_random_keys[
                                project_name
                                ...
                                + "-**-"
                                + key
                            ] = doc_random_keys[filename]["annotations"][annotations][
                                key
                            ]
    ...
    logging.info(
        msg="Processing of given projects done! Timestamp key from this run: "
        + timestamp_key
    )
...
def set_surrogates_in_inception_files(config):
    ...
    project_surrogate = (
        dir_out_public + os.sep + "surrogate" + "_" + project_name + "_" + timestamp_key
    )
    if not os.path.exists(path=project_surrogate):
        os.makedirs(name=project_surrogate)
    ...
    dir_project_cas = (
        dir_project_private + os.sep + "cas" + "_" + project_name + "_" + timestamp_key
    )
    if not os.path.exists(path=dir_project_cas):
        os.makedirs(name=dir_project_cas)
    ...
    for mode in surrogate_modes:  ## eigentlich nur 1 Modus!!
        ...
        for ann_doc in os.listdir(path_files_to_process):
            if ann_doc.endswith("json"):  # or cas_file.endswith('xmi'):
                with open(
                    path_files_to_process + os.sep + ann_doc, "rb"
                ) as cas_file_stream:
                    cas = cassis.load_cas_from_json(cas_file_stream)

                logging.info(
                    msg="processing file: " + path_files_to_process + os.sep + ann_doc
                )
                m_cas = deepcopy(cas)

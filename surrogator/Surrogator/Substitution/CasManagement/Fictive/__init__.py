...
from Surrogator.Substitution.Entities.Location.Location_Hospital import (
    load_hospital_names,
)
from Surrogator.Substitution.Entities.Location.Location_Hospital import (
    get_hospital_surrogate,
)
from Surrogator.Substitution.Entities.Location.Location_address import (
    get_address_location_surrogate,
)
from Surrogator.Substitution.Entities.Location.Location_orga_other import (
    load_location_names,
)
from Surrogator.Substitution.Entities.Location.Location_orga_other import (
...
class CasManagementFictive(CasManagement):
    ...
    def load_nn_and_resource(self, nn_path: str, data_path: str, data_loader_fn):
        """
    ...
    def manipulate_cas(self, cas):
                    ...
                    if custom_pii.kind not in ["PROFESSION", "AGE"]:
                        if custom_pii.kind in {
                            ...
                            if custom_pii.get_covered_text() not in names.keys():
                                ...
                                preceding_tokens = (
                                    preceding_tokens[-5:]
                                    if len(preceding_tokens) >= 5
                                    else preceding_tokens
                                )
                        ...
                        if custom_pii.kind in ["DATE", "DATE_BIRTH", "DATE_DEATH"]:
                            dates[custom_pii.get_covered_text()] = (
                                custom_pii.get_covered_text()
                            )
                        ...
                        if custom_pii.kind == "LOCATION_HOSPITAL":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_location_hospitals.keys()
                            ):
                                hospitals[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )
                        if custom_pii.kind == "LOCATION_ORGANIZATION":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_location_organizations.keys()
                            ):
                                organizations[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )
                        if custom_pii.kind == "LOCATION_OTHER":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_location_replaced_others.keys()
                            ):
                                others[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )
                        if custom_pii.kind == "LOCATION_COUNTRY":
                            if custom_pii.get_covered_text() not in countries.keys():
                                countries[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )
                        ...
                        if custom_pii.kind == "LOCATION_STATE":
                            ...
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_location_replaced_address_locations
                            ):
                        ...
                        if custom_pii.kind == "ID":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_identifiers.keys()
                            ):
                                identifiers[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )

                        if (
                            custom_pii.kind == "CONTACT_PHONE"
                            or custom_pii.kind == "CONTACT_FAX"
                        ):
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_contact_phone_numbers
                            ):
                        ...
                        if custom_pii.kind == "CONTACT_EMAIL":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_contact_email.keys()
                            ):
                                contacts_email[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )
                        ...
                        if custom_pii.kind == "CONTACT_URL":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_contact_url.keys()
                            ):
                                contacts_url[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )
                        ...
                        if custom_pii.kind == "NAME_USER":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_user_names.keys()
                            ):
                                user_names[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )
                        ...
                        if custom_pii.kind == "NAME_TITLE":
                            if (
                                custom_pii.get_covered_text()
                                not in self.global_name_titles.keys()
                            ):
                                titles[custom_pii.get_covered_text()] = (
                                    custom_pii.get_covered_text()
                                )

                elif custom_pii.kind is None:
                    logging.warning(
                        "token.kind: NONE - " + custom_pii.get_covered_text()
                    )
        ...
        self.global_contact_phone_numbers.update(
        ...
        self.global_location_replaced_address_locations.update(
            get_address_location_surrogate(
                overpass_api, states, cities, streets, zips, area_codes, tel_dict
            )
        )
        ...
        for area_code in area_codes:
            if area_code not in self.global_location_replaced_address_locations:
                self.global_location_replaced_address_locations[area_code] = (
                    random.choice(MOBILE_PREFIXES)
                )
        ...
        other_nn, other_names = self.load_nn_and_resource(
            OTHER_NEAREST_NEIGHBORS_MODEL_PATH, OTHER_DATA_PATH, load_location_names
        )
        ...
        relevant_types = [
            t for t in cas.typesystem.get_types() if "PHI" in t.name
        ]  # do not rename this PHI mention!
        cas_name = relevant_types[0].name
                ...
                if custom_pii.kind is not None:
                    if custom_pii.kind not in ["PROFESSION", "AGE"]:
                        if custom_pii.kind in {
                            ...
                            replace_element = self.global_names[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            )

                        elif custom_pii.kind == "NAME_USER":
                            replace_element = self.global_user_names[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            replace_element = self.global_dates[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            if self.date_shift == 0:
                                quarter_date = get_quarter(
                                    custom_pii.get_covered_text()
                                )
                                replace_element = quarter_date
                                key_ass_ret[custom_pii.kind][quarter_date] = (
                                    custom_pii.get_covered_text()
                                )
                            else:
                                replace_element = self.global_dates[
                            ...
                            replace_element = self.global_location_hospitals[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            replace_element = self.global_location_organizations[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            replace_element = self.global_location_replaced_others[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            if (
                                custom_pii.get_covered_text()
                                in self.global_location_replaced_address_locations.keys()
                            ):
                                replace_element = (
                                    self.global_location_replaced_address_locations[
                                        custom_pii.get_covered_text()
                            ...
                            replace_element = self.global_identifiers[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            replace_element = self.global_contact_phone_numbers[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            replace_element = self.global_contact_email[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
                            ...
                            replace_element = self.global_contact_url[
                                custom_pii.get_covered_text()
                            ]
                            key_ass_ret[custom_pii.kind][replace_element] = (
                                custom_pii.get_covered_text()
        ...
        return {
            "cas": self.manipulate_sofa_string_in_cas(
                cas=cas, new_text=new_text, shift=shift
            ),
            "key_ass": key_ass_ret,

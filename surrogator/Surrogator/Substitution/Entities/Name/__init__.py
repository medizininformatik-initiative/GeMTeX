...
with open(
    "resources" + os.sep + "de_subLists" + os.sep + "male.json", "r", encoding="utf-8"
) as male_file:
    male_data = json.load(male_file)

with open(
    "resources" + os.sep + "de_subLists" + os.sep + "female.json", "r", encoding="utf-8"
) as female_file:
    female_data = json.load(female_file)

with open(
    "resources" + os.sep + "de_subLists" + os.sep + "family.json", "r", encoding="utf-8"
) as family_file:
...
def surrogate_names_by_fictive_names(list_of_names):
    ...
    male_df = pd.DataFrame(
        [name for _, names in male_data.items() for name in names], columns=["Name"]
    )
    female_df = pd.DataFrame(
        [name for _, names in female_data.items() for name in names], columns=["Name"]
    )
    ...
    family_df = pd.DataFrame(
        [name for _, names in family_data.items() for name in names], columns=["Name"]
    )
            ...
            if classification_value == "FN":
                if key_norm not in surrogate_all:
                    gender = detect_gender(
                        classification_key, preceding_words, gender_guesser
                    )

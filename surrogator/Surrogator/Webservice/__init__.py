...
from Surrogator.QualityControl import (
    run_quality_control_of_project,
...
def select_method_to_handle_the_data():
    ...
    modus = st.sidebar.radio(
        "Choose the surrogation modus:", ("x", "entity", "gemtex", "fictive")
    )
    ...
    if method == "Manually":
        ...
        if button_qc_m:
            if uploaded_files:
                ...
                st.session_state["projects"] = read_dir(
                    str(upload_folder), selected_projects
                )
        ...
        if inception_status and "available_projects" in st.session_state:
            ...
            if button_qc_a or button_sur_a:
                ...
                st.session_state["projects"] = read_dir(
                    dir_path=projects_folder, selected_projects=selected_projects_names
                )
            ...
            if button_sur_a:
                ...
                config["surrogate_process"]["surrogate_modes"].append(
                    modus
                )  # .append("gemtex") ## todo
                config["surrogate_process"]["rename_files"] = True
                st.session_state["config"] = config
...
def create_zip_download_quality_control(project_name, timestamp_key, paths_reports):
    ...
    with open(
        os.getcwd() + os.sep + paths_reports["dir_project_quality_control"] + ".zip",
        "rb",
    ) as zip_qc_file:
...
def webservice_output_quality_control(quality_control, timestamp_key, project_name):
    ...
    st.write(
        "<h3> Excluded Documents from Corpus (containing OTHER or NONE annotation)</h3>",
        unsafe_allow_html=True,
    )
...
def main():
    ...
    if (
        "projects" not in st.session_state.keys()
        or (len(st.session_state["projects"]) == 0)
        or st.session_state["projects"] != []
    ):
        st.write("<b>Input a valid project path.</b>", unsafe_allow_html=True)

    if (
        "method" in st.session_state.keys()
        and "projects" in st.session_state.keys()
        and len(st.session_state["projects"]) > 0
        and "config" not in st.session_state.keys()
    ):
        ...
        for project in projects:
            ...
            st.write("<hr>", unsafe_allow_html=True)
            project_name = "-".join(
                project["name"].replace(".zip", "").split("-")[0:-1]
            )
    ...
    if "config" in st.session_state and "projects" in st.session_state:
        ...
        surrogate_return = set_surrogates_in_inception_projects(
        ...
        if surrogate_return == 0:
            ...
            st.write(
                "<b>timestamp_key:</b> " + str(timestamp_key), unsafe_allow_html=True
            )
            ...
            for proj in projects:
                ...
                webservice_output_quality_control(
                    quality_control=surrogate_return["quality_control_of_projects"][
                        proj
                    ],  ## hier gibts Fehler.
                    timestamp_key=timestamp_key,
                    project_name=proj,
            ...
            st.write(
                "<b>Private exports:</b> " + str(dir_out_private),
                unsafe_allow_html=True,
            )
            st.write(
                "<b>Public exports:</b> " + str(dir_out_public), unsafe_allow_html=True
            )
            ...
            ste.download_button(
                label="Download PRIVATE files: cas annotation files and statistics ("
                + timestamp_key
                + ")",
                data=zip_byte,
            ...
            ste.download_button(
                label="Download PUBLIC files: text files with surrogates ("
                + timestamp_key
                + ")",
